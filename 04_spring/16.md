...
Nesta etapa, começaremos a construir o Recurso Usuário, que será composto por duas classes principais: **`Usuario`** e **`UsuarioLogin`**.

<div align="center"><img src="https://i.imgur.com/1OVel2T.png" title="source: imgur.com" width="70%"/></div>

A **classe Usuario** servirá de modelo para construir a tabela **tb_usuarios** (Entidade) dentro do nosso Banco de dados **db_blogpessoal**. Os campos (Atributos) da tabela serão os mesmos que estão definidos no Diagrama de classes acima. Depois de criar a classe Model Usuario, vamos executar o projeto Blog Pessoal no STS. Após a execução veremos que será criado no Banco de dados **db_blogpessoal** a tabela **tb_usuarios**. 
...
Note que a **Query Method** utilizada na `UsuarioRepository` é diferente das utilizadas nas interfaces `PostagemRepository` e `TemaRepository`, pois ela buscará um usuário por seu e-mail (atributo `usuario`).
...
A classe `Postagem` será o lado **N:1** da relação, ou seja, **muitas postagens podem estar associadas a apenas um usuário**.

Para estabelecer essa relação, vamos inserir as três linhas abaixo na classe `Postagem`, **logo após o último atributo da classe: `tema`**:
...
Na sequência, não esqueça de atualizar os **Métodos Get e Set**, acrescentando os respetivos métodos para o novo Atributo (usuario), que foi adicionado na classe Postagem.
